...
public class WebServiceConnectionManager {
   ...
   public void start(String options) throws HypervisorException {
      ...
      try {
         ...
         if (!vboxManager.getVBox().getAPIVersion().contentEquals("4_2")) {
            throw new HypervisorException("Missmatch API Connector: Server is " + vboxManager.getVBox().getAPIVersion() + " but the connector handles 4_2");
         }
   ...
   private void connect(String host, Integer port, String username, String pass) {
      ...
      vboxManager.connect(connInfo, username, pass);
   }
