...
import org.altherian.hboxd.vbox.hypervisor.ws4_2.vm.guest.VBoxGuest;
...
public final class VBoxMachine implements _RawVM {
   ...
   private void unlockAuto() {
      VbSessionManager.get().unlockAuto(getUuid(), false);
      session = null;
   }
   ...
   public _RawSnapshot takeSnapshot(String name, String description) {
      ...
      try {
         ...
         Logger.debug("VBox API Return code: " + p.getResultCode());
         if (p.getResultCode() == 0) {
            _RawSnapshot rawSnap = getSnapshot(name);
            EventManager.post(new SnapshotTakenEvent(uuid, rawSnap.getUuid()));
            return rawSnap;
         } else {
            throw new MachineException(p.getErrorInfo().getText());
         }
   ...
   public void deleteSnapshot(String id) {
      ...
      try {
         ...
         Logger.debug("VBox API Return code: " + p.getResultCode());
         if (p.getResultCode() == 0) {
            EventManager.post(new SnapshotDeletedEvent(uuid, id));
         } else {
            throw new MachineException(p.getErrorInfo().getText());
         }
      } catch (VBoxException e) {
   ...
   public byte[] takeScreenshot() {
      ...
      try {
         ...
         session.getConsole().getDisplay().getScreenResolution(0l, width, height, bpp);
