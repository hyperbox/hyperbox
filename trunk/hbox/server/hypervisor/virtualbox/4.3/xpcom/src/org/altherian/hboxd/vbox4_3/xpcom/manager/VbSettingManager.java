...
import org.altherian.hboxd.vbox.hypervisor.xpcom4_3.storage.VirtualboxMedium;
import org.altherian.hboxd.vbox.hypervisor.xpcom4_3.storage.VirtualboxStorageController;
import org.altherian.hboxd.vbox.hypervisor.xpcom4_3.vm.VBoxMachine;
import org.altherian.hboxd.vbox.hypervisor.xpcom4_3.vm.VBoxNetworkInterface;
import org.altherian.hboxd.vbox.hypervisor.xpcom4_3.vm.VBoxSnapshot;
...
import org.altherian.hboxd.vbox4_3.xpcom.setting._MediumSettingAction;
import org.altherian.hboxd.vbox4_3.xpcom.setting._NetworkInterfaceSettingAction;
import org.altherian.hboxd.vbox4_3.xpcom.setting._SnapshotSettingAction;
import org.altherian.hboxd.vbox4_3.xpcom.setting._StorageControllerSettingAction;
...
public class VbSettingManager {
   ...
   private static void lockAuto(String uuid) {
      session = VbSessionManager.get().lockAuto(uuid);
   }
   
   private static void unlockAuto(String uuid) {
      VbSessionManager.get().unlockAuto(uuid, true);
      session = null;
   }
   
   private static IMachine getVm(String uuid) {
      return VbSessionManager.get().getCurrent(uuid);
   }
   ...
   public static List<_Setting> list(VBoxSnapshot snap) {
      ...
      for (_SnapshotSettingAction action : VbSettingManager.snapshotActions.values()) {
   ...
   public static void apply(IMachine vm, Machine vmData) {
      ...
      for (Device dev : vmData.listDevices(EntityTypes.Network.getId())) {
         Long maxNic = ConnectionManager.getBox().getSystemProperties().getMaxNetworkAdapters(vm.getChipsetType());
   ...
   public static List<_Setting> list(VBoxMachine vm) {
      ...
      for (_MachineSettingAction action : VbSettingManager.vmActions.values()) {
   ...
   public static _Setting get(VBoxMachine vm, Object name) {
      String uniqueId = Settings.getUniqueId(name);
      if (!VbSettingManager.vmActions.containsKey(uniqueId)) {
         throw new ConfigurationException("No action defined for setting [" + uniqueId + "]");
      }
      
      IMachine machine = getVm(vm.getUuid());
      _Setting s = VbSettingManager.vmActions.get(uniqueId).get(machine);
   ...
   public static void set(VBoxNetworkInterface nic, List<_Setting> settings) {
      ...
      try {
         for (_Setting setting : settings) {
            Logger.debug("Applying Setting ID [" + setting.getName() + "]");
            _NetworkInterfaceSettingAction settingAction = VbSettingManager.nicActions.get(setting.getName());
   ...
   public static List<_Setting> list(VirtualboxMedium medium) {
      List<_Setting> settings = new ArrayList<_Setting>();
      IMedium rawMedium = ConnectionManager.getBox().openMedium(medium.getUuid(), DeviceType.valueOf(medium.getDeviceType()), AccessMode.ReadOnly, false);
   ...
   public static void set(VirtualboxMedium medium, List<_Setting> settings) {
      ...
      IMedium rawMedium = ConnectionManager.getBox().openMedium(medium.getLocation(), DeviceType.valueOf(medium.getDeviceType()), AccessMode.ReadWrite, false);
      try {
         for (_Setting setting : settings) {
            Logger.debug("Applying Setting ID [" + setting.getName() + "]");
   ...
   public static _Setting get(VirtualboxMedium medium, Object name) {
      ...
      IMedium rawMedium = ConnectionManager.getBox().openMedium(medium.getLocation(), DeviceType.valueOf(medium.getDeviceType()), AccessMode.ReadOnly, false);
      try {
